...
jest.spyOn(React, "useEffect").mockImplementation((f, metrics) => {
  if (!arrayEqual(prevMetrics, metrics)) {
    f();
  }
  prevMetrics = metrics;
});
...
let wrapper;
beforeEach(() => {
  wrapper = shallow(<Game size={3} onReset={() => {}} />);
});
...
test("Scenario: First marker is placed", () => {
  ...
  wrapper
    .find("Board")
    .simulate("action", testData.xCoordinate, testData.yCoordinate);
  const newBoard = wrapper.find("Board").prop("values");
  expect(newBoard[testData.xCoordinate][testData.yCoordinate]).toEqual(
    playerOnTurn
  );
});
...
test("Scenario: Player 1 wins the game", () => {
  ...
  expect(wrapper.find("Board").prop("values")).toEqual([
    [1, 0, 0],
    [2, 1, 0],
    [2, 0, 1]
  ]);
